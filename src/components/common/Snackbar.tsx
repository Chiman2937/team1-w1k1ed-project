...
const SnackBar = ({ variant, isClosing, isOpening, index, children, className }: Props) => {
  const iconMap = {
    info: IconInfo,
    success: IconSuccess,
    error: IconError,
  ...
  const containerStyle = clsx(
    'fixed left-[50%] translate-x-[-50%] whitespace-nowrap',
  ...
  return (
    <div
      className={twMerge(clsx(containerStyle, variantStyle[variant], animationStyle, className))}
    >
      <IconComponent />
      {children}
    </div>
